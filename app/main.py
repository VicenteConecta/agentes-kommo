...
async def webhook_lead_added(request: Request):
    """
    Webhook para cuando se agrega un nuevo lead.
    Dispara el Agente Calificador (Bloque 1).
    Acepta cualquier estructura JSON y la procesa de forma flexible.
    """
    try:
        # Leer el body completo del request
        body = await request.json()
        logger.info(f"📥 WEBHOOK RAW BODY: {body}")
        logger.info(f"📥 WEBHOOK BODY TYPE: {type(body)}")
        
        # Intentar extraer el lead_id de diferentes estructuras posibles
        lead_id = None
        lead_data = {}
        
        # Estructura 1: {"leads": {"add": [{"id": ..., ...}]}}
        if isinstance(body, dict) and "leads" in body:
            if isinstance(body["leads"], dict) and "add" in body["leads"]:
                if isinstance(body["leads"]["add"], list) and len(body["leads"]["add"]) > 0:
                    lead_data = body["leads"]["add"][0]
                    lead_id = lead_data.get("id")
                    logger.info(f"✅ Estructura detectada: leads.add[0]")
        
        # Estructura 2: {"data": {"id": ...}} o {"event": ..., "data": {...}}
        if not lead_id and isinstance(body, dict) and "data" in body:
            lead_data = body["data"]
            lead_id = lead_data.get("id")
            logger.info(f"✅ Estructura detectada: data.id")
        
        # Estructura 3: {"id": ...} (directo)
        if not lead_id and isinstance(body, dict) and "id" in body:
            lead_data = body
            lead_id = body.get("id")
            logger.info(f"✅ Estructura detectada: id directo")
        
        # Si aún no encontramos el lead_id, loguear y devolver error descriptivo
        if not lead_id:
            logger.error(f"❌ No se pudo extraer lead_id del payload: {body}")
            return JSONResponse(
                status_code=400,
                content={
        ...
        logger.info(f"✅ Lead ID extraído: {lead_id}")
        ...
        logger.error(f"❌ Error procesando webhook lead-added: {e}", exc_info=True)
        return JSONResponse(
            status_code=500,
            content={"error": str(e)}
        )
